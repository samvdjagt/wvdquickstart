...
* <a href="https://github.com/samvdjagt/wvdquickstart/tree/master/ARMRunbookScripts/inputValidation.ps1" target="_blank">inputValidation.ps1</a>: This script makes sure that the entered Azure Admin credentials are correct, in addition to validating the vnet, subnet, and firewall configuration. This script also configures the 'WVDServicePrincipal' managed identity in the deployment resource group to give it the *contributor* role on the subscription. This is needed to run deployment scripts in the main ARM template successfully.
  * <a href="https://github.com/samvdjagt/wvdquickstart/tree/master/ARMRunbookScripts/AADDSinputValidation.ps1" target="_blank">AADDSinputValidation.ps1</a>: This script is an adapted version of the script above, and is only used when using the QuickStart starting with an empty subscription.
